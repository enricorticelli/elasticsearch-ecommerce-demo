...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    fetchProducts();
  }, [API_URL, category, page, pageSize, products.length, sort]);
  ...
  const updateParams = (updates: Record<string, string>) => {
    ...
    startTransition(() => {
      router.replace(`/categories/${encodeURIComponent(category)}?${params.toString()}`, { scroll: false });
    });
  };

  const totalPages = Math.max(1, Math.ceil(total / pageSize));
  const title = useMemo(() => (category ? `Categoria: ${category}` : 'Categoria'), [category]);

  return (
    <div className="mx-auto max-w-6xl px-6 pb-16 pt-10">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-sm text-[var(--color-ash)]">Categoria</p>
